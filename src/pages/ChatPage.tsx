...
const RECOMMENDED_QUESTIONS = ["What are the best exercises for heart health?", "How can I improve my sleep quality?", "What foods should I eat to boost my immunity?", "How do I manage stress effectively?", "What are common symptoms of vitamin deficiency?"];
          ...
          <div className="bg-white/80 dark:bg-gray-900/80 backdrop-blur-md rounded-2xl shadow-xl flex-1 p-6 mb-6 overflow-hidden flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto mb-6 space-y-6">
              {messages.map(message => <div key={message.id} className={`flex ${message.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[80%] rounded-2xl p-4 ${message.sender === 'user' ? 'bg-primary text-primary-foreground' : 'bg-secondary text-secondary-foreground'}`}>
                    <p>{message.content}</p>
            ...
            {messages.length === 1 && <div className="mb-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {RECOMMENDED_QUESTIONS.map((question, index) => <Button key={index} variant="outline" size="sm" className="rounded-full text-xs" onClick={() => handleQuestionClick(question)}>
                      {question}
                    </Button>)}
                </div>
              </div>}
          ...
          <div className="bg-white dark:bg-gray-900 rounded-full shadow-md border border-gray-200 dark:border-gray-700">
            <div className="flex items-center px-4 py-1">
              ...
              <div className="flex-1 mx-2">
                <AIInputWithSuggestions placeholder="Ask me anything about your health..." onSubmit={handleSubmit} actions={HEALTH_ACTIONS} maxHeight={200} className="py-2" />
              </div>
