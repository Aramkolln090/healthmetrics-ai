            ...
            <div className="container mx-auto h-full px-4 py-4 flex flex-col">
              <div className="bg-white/80 dark:bg-gray-900/80 backdrop-blur-md rounded-2xl shadow-xl flex-1 p-6 overflow-hidden flex flex-col">
                ...
                <div className="flex-1 overflow-y-auto mb-6 space-y-6">
                  {currentMessages.map(message => <div key={message.id} className={`flex ${message.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-[80%] rounded-2xl p-4 ${message.sender === 'user' ? 'bg-primary text-primary-foreground' : 'bg-secondary/80 text-secondary-foreground'}`}>
                        <p>{message.content}</p>
                ...
                {/* Suggested questions */}
                {currentMessages.length <= 2 && <div className="mb-6">
                    <h3 className="text-sm font-medium text-muted-foreground mb-3">Suggested questions:</h3>
                    <div className="flex flex-wrap gap-2">
                      {SUGGESTED_QUESTIONS.map((question, index) => <button key={index} onClick={() => handleSuggestedQuestionClick(question)} className="px-3 py-2 text-sm bg-secondary/50 hover:bg-secondary/70 text-secondary-foreground rounded-full transition-colors">
                          {question}
                        </button>)}
                    </div>
                  </div>}
                ...
                <div className="mt-auto relative">
                  ...
                  ease: "easeOut"
                    ...
                    <GlowEffect colors={['#0894FF', '#C959DD', '#FF2E54', '#FF9004']} mode="colorShift" blur="medium" scale={1.1} duration={3} />
                  </motion.div>
                  <AIInputWithSuggestions placeholder="Ask me anything about your health..." onSubmit={handleSubmit} actions={HEALTH_ACTIONS} maxHeight={200} className="relative z-10" />
                </div>
              </div>
            </div>
          </div>
